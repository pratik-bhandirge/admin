...
class WebsiteSale(WebsiteSale):
    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        print("values['products']----",values['products'])
        if not values['products']:
            search_lst = search.split()
            print("search_lst----",search_lst)
            if search_lst:
                search_string = []
                for search_lst_value in search_lst:
                    search_string += [spell.correction(search_lst_value)]
                search_string = " ".join(search_string)                
                if search_string == search_lst:
                    search_string = []                    
                    for search_lst_value in search_lst:
                        correction = d.suggest(search_lst_value)
                        search_string += correction[0] if correction else spell.correction(search_lst_value)
                search_string = " ".join(search_string)
                print("search_string----",search_string)
                if search_string != search_lst:
                    values = self.shop_copy(page, category, search_string, ppg)
                # values['products'] = products
                # values['attributes'] = attributes
        ddddd
        return request.render("website_sale.products", values)

    def shop_copy(self, page=0, category=None, search='', ppg=False, **post):
        print("category----****------------",category)
        print("page-------***---------",page)
        print("ppg------------***----",ppg)
        print("search-----****-----------",search)
