    ...
    def shop(self, page=0, category=None, search='', ppg=False, **post):
        ...
        if current_website.website_shop_login and (request.env.user._is_public() or request.env.user.id == request.website.user_id.id):
            ...
            if current_website.website_shop_login_redirect:
                redirect_url = '%s?redirect=%s' % (
                    current_website.website_shop_login_redirect, request.httprequest.url)
            else:
                # redirct user to /web/signup if b2c signup is enable
                if current_website.website_auth_signup_uninvited == 'b2c':
                    redirect_url = '/web/signup?redirect=%s' % (
        ...
        # Add following condition to handle wrong input seach string
        if not products:
            product_ids = []
            for search_l in search.split():
                domain = self._get_search_domain(
                    search_l, category, attrib_values)
                product_ids += Product.search(domain).ids
                if not d.check(search_l):
                    suggested_search = d.suggest(search_l)
                    if suggested_search:
                        for search_c in suggested_search:
                            domain = self._get_search_domain(
                                search_c, category, attrib_values)
                            product_ids += Product.search(domain).ids

            if product_ids:
                domain = [('id', 'in', product_ids)]
                domain += [('public_categ_ids', 'child_of',
                            [x.id for x in categs])]
            else:
                domain = [('public_categ_ids', 'child_of',
                           [x.id for x in categs])]
